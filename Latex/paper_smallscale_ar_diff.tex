...
%DIF DEL paper_smallscale_ar.tex                                                                            Fri Dec  9 22:22:29 2016
%DIF ADD /Users/hoonhout/Checkouts/PhD/Journals/fielddata/AeolianResearch/smallscale_ar/smallscale_ar.tex   Thu Sep  1 17:10:06 2016
...
%DIF 9d7
%DIF < % add references to research questions in margin
%DIF -------
\newcommand{\mrq}[2][]{}
\newcommand{\rrq}[3][]{}
\newboolean{ispaper}
...
\begin{frontmatter}
  ...
  \DIFdelbegin %DIFDELCMD < \author[tudelft,deltares]{Bas Hoonhout\corref{ca}}
%DIFDELCMD <   \ead{b.m.hoonhout@tudelft.nl}
%DIFDELCMD <   \ead{bas.hoonhout@deltares.nl}
%DIFDELCMD <   \cortext[ca]{Corresponding author}
%DIFDELCMD < %%%
\DIFdelend %DIF > \author[tudelft,deltares]{Bas Hoonhout\corref{ca}}
  %DIF > \ead{b.m.hoonhout@tudelft.nl}
  %DIF > \ead{bas.hoonhout@deltares.nl}
  %DIF > \cortext[ca]{Corresponding author}
  %DIF > 
  %DIF > \author[tudelft]{Sierd de Vries}
  %DIF > 
  %DIF > \address[tudelft]{Delft University of Technology, Faculty of Civil
  %DIF >   Engineering and Geosciences, Department of Hydraulic Engineering,
  %DIF >   Stevinweg 1, 2628CN Delft, The Netherlands.}
  %DIF > 
  %DIF > \address[deltares]{Deltares, Department of Hydraulic Engineering,
  %DIF >   Boussinesqweg 1, 2629HV Delft, The Netherlands.}

  \DIFdelbegin %DIFDELCMD < \author[tudelft]{Sierd de Vries}
%DIFDELCMD < 

%DIFDELCMD <   \address[tudelft]{Delft University of Technology, Faculty of Civil
%DIFDELCMD <     Engineering and Geosciences, Department of Hydraulic Engineering,
%DIFDELCMD <     Stevinweg 1, 2628CN Delft, The Netherlands.}
%DIFDELCMD < 

%DIFDELCMD <   \address[deltares]{Deltares, Department of Hydraulic Engineering,
%DIFDELCMD <     Boussinesqweg 1, 2629HV Delft, The Netherlands.}
%DIFDELCMD < 
...
spatially aggregated \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Jackson1998, Arens2001, Wiggs2004}}%DIFAUXCMD
. The
...
aggregation\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{deVries2014a, Hoonhout2016}}%DIFAUXCMD
...
sediment. \DIFaddbegin \DIFadd{Local variations in sediment availability influence the
aeolian sediment supply and consequently aeolian sediment
transport. }\DIFaddend Temporal variations in aeolian sediment transport are
\DIFdelbegin \DIFdel{still
}\DIFdelend \DIFaddbegin \DIFadd{therefore }\DIFaddend expected to be correlated with the wind speed, but spatial
...
ultimately helps improving gross aeolian sediment transport \DIFdelbegin \DIFdel{estimates }\DIFdelend \DIFaddbegin \DIFadd{rate }\DIFaddend in
availability-limited coastal systems.

\section{Field Site}
\DIFdelbegin %DIFDELCMD < \label{sec:fieldsite2}
%DIFDELCMD < %%%
\DIFdelend 

The Sand Motor mega nourishment was constructed in 2011 along the
Delfland coast in The Netherlands \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[Figure
\ref{fig:fieldsite2},][]{Stive2013}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[Figure \ref{fig:map} and
\ref{fig:fieldsite},][]{Stive2013}}%DIFAUXCMD
}\DIFaddend .  The Delfland coast was originally
...
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\columnwidth]{../Figures/location_and_evolution}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Location, orientation, appearance and evolution of the Sand
    ...
    as reference.}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:fieldsite2}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \ifthenelse{\boolean{ispaper}}{
\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{map}
  \caption{Location of the Sand Motor field site.}
  \label{fig:map}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=\columnwidth]{fieldsite}
  \caption{Evolution of the Sand Motor since construction in
    2011. Upper panels: aerial photographs courtesy of
    Rijkswaterstaat/Joop van Houdt. Lower panels: evolution of 1 m+MSL
    contour lines.}
  \label{fig:fieldsite}
\end{figure}
}{}
...
%DIFDELCMD < \noindent %%%
\DIFdel{in which $u$ is the wind speed, is }\DIFdelend \DIFaddbegin \ifthenelse{\boolean{ispaper}}{
($\Psi$), defined as:
...
\noindent in which $u$ is the wind speed,}{
(Equation \ref{eq:transport_potential})}
...
%DIF < The tidal channel that connects the lagoon to the sea elongated and
%DIF < started to meander. Consequently, the 2 m tidal range that is present
%DIF < at the outer contour of the Sand Motor is damped over time to
%DIF < approximately 20 cm in the lagoon \citep{deVries2015}. At the closed
%DIF < end of the lagoon the tidal currents are therefore
%DIF < negligible. Consequently, morphological change in this area can be
%DIF < associated with aeolian sediment transport as is the morphological
%DIF < change in the dune lake.  The lagoon and dune lake therefore act as
%DIF < large sediment traps for aeolian sediment.
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/overview}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{overview}
  \DIFaddendFL \caption{Overview of measurement transects N, W, and SW and
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/mast_small}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{mast_small}
  \DIFaddendFL \caption{Mast with 6 Wenglor fork laser sensors and a Gill 2D
    WindSonic ultrasonic wind speed and direction sensor\DIFdelbeginFL \DIFdelFL{viewed in
    direction of the wind}\DIFdelendFL . The top 3
...
above the bed (Figure \ref{fig:mast}). \DIFaddbegin \DIFadd{The lowest sensor was buried as
to minimize the disturbance of the wind field. }\DIFaddend An additional three
laser sensors were added to the most landward mast at 40, 55 and 70 cm
above the bed to estimate the amount of particles \DIFdelbegin \DIFdel{bypassing }\DIFdelend \DIFaddbegin \DIFadd{surpassing }\DIFaddend the lower
three sensors. Other masts could be equipped with three additional
laser sensors as well. \DIFdelbegin \DIFdel{All except the lowest sensor were placed horizontally
...
disturbance of the wind field. The }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend Wenglor fork laser sensors register passing
...
related to the sediment flux \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Hugenholtz2011}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Barchyn2014}}%DIFAUXCMD
}\DIFaddend , both are used
...
\mbox{%DIFAUXCMD
\citet{Barchyn2014}}%DIFAUXCMD
:
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
  \DIFdel{q_{\mathrm{wenglor}} = n_{\mathrm{wenglor}} \left( 
    \frac{6 \cdot \gamma}{\rho \pi D^3} \cdot l_{\mathrm{fork}} \cdot \left(
      l_{\mathrm{laser}} + D}\end{displaymath} %DIFAUXCMD
...
%DIFDELCMD < \noindent %%%
\DIFdel{with $\rho = 2650$ $\mathrm{kg/m^3}$,
...
%DIFDELCMD < %%%
\DIFdelend Variations in wind direction of more than $45^o$ resulted in
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/particlecounts_timeseries}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{particlecounts_timeseries}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{a) }\DIFdelendFL Wind time series \DIFdelbeginFL \DIFdelFL{, b}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(upper panel}\DIFaddendFL )\DIFaddbeginFL \DIFaddFL{, }\DIFaddendFL overall particle count
    rates during the deployments along the westerly transect \DIFdelbeginFL \DIFdelFL{, and c}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(middle
    panel}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL offshore tidal elevation \DIFaddbeginFL \DIFaddFL{(lower panel)}\DIFaddendFL . Grey lines
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/particlecounts_correlations}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{particlecounts_correlations}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{a) }\DIFdelendFL Relations between overall particle count and wind speed
    \DIFdelbeginFL \DIFdelFL{or b}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(left panel}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{or }\DIFaddendFL water level \DIFaddbeginFL \DIFaddFL{(right panel)}\DIFaddendFL . Closed circles and
...
and sunny weather and negligible precipitation\DIFdelbegin \DIFdel{, which is unusual for
the time of the year}\DIFdelend . The average wind speed
over the entire experiment was 6 m/s (Figure \ref{fig:pc_timeseries}\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{,
upper panel}\DIFaddend ). The maximum wind speed was registered at 24 m/s at the
...
experiment was 120 $\mathrm{s^{-1}}$ \DIFdelbegin \DIFdel{or $< 0.1$ $\mathrm{kg/m^2/s}$ }\DIFdelend averaged over all deployed
sensors (Figure \ref{fig:pc_timeseries}\DIFdelbegin \DIFdel{b}\DIFdelend \DIFaddbegin \DIFadd{, middle panel}\DIFaddend ). The maximum
overall particle count rate was registered on October 7 at 5800
$\mathrm{s^{-1}}$ \DIFdelbegin \DIFdel{or 4 $\mathrm{kg/m^2/s}$ }\DIFdelend (DN06b). Therefore, the maximum registered overall
...
varying between 1.5 and 2.0 m (Figure \ref{fig:pc_timeseries}\DIFdelbegin \DIFdel{c}\DIFdelend \DIFaddbegin \DIFadd{, lower
panel}\DIFaddend ). The maximum still water level of 2.8 m+MSL was measured during
...
particle count (Figure \ref{fig:pc_timeseries}\DIFdelbegin \DIFdel{a, b}\DIFdelend \DIFaddbegin \DIFadd{, upper and middle
panel}\DIFaddend ). Also the occurrence of peaks in overall particle count show a
...
\ref{fig:pc_correlations}\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{, left panel}\DIFaddend ). The poor correlation is
...
count is found (Figure \ref{fig:pc_correlations}\DIFdelbegin \DIFdel{b}\DIFdelend \DIFaddbegin \DIFadd{, right panel}\DIFaddend ). There
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=.7\columnwidth]{../Figures/polar_72096_451703}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=.7\columnwidth]{polar_72096_451703}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{a) }\DIFdelendFL Per-mast particle count, wind speed and direction obtained
    from stationary mast (Figure \ref{fig:overview}\DIFaddbeginFL \DIFaddFL{, upper panel}\DIFaddendFL ) and
    \DIFdelbeginFL \DIFdelFL{b)
    }\DIFdelendFL available fetch and intertidal fetches \DIFaddbeginFL \DIFaddFL{(lower panel)}\DIFaddendFL .}
...
the largest transport potential (Figure \ref{fig:pc_direction}\DIFdelbegin \DIFdel{a}\DIFdelend \DIFaddbegin \DIFadd{, upper
panel}\DIFaddend ).

\DIFdelbegin \DIFdel{Figure \ref{fig:pc_direction}a }\DIFdelend \DIFaddbegin \DIFadd{The upper panel in Figure \ref{fig:pc_direction} }\DIFaddend shows that the
...
\DIFdelbegin \DIFdel{Figure \ref{fig:pc_direction}b }\DIFdelend \DIFaddbegin \DIFadd{The lower panel in Figure \ref{fig:pc_direction} }\DIFaddend shows that most
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/particlecounts_transects}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{particlecounts_transects}
  \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{a) }\DIFdelendFL Average per-mast particle count rates during the deployments
    along the westerly transect \DIFdelbeginFL \DIFdelFL{and b}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(upper panel}\DIFaddendFL ) \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL beach profile \DIFdelbeginFL \DIFdelFL{at
    the beginning of the field campaign}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{(lower
    panel)}\DIFaddendFL . Line colors refer to the partitioning of the time series
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/particlecounts_DN06}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{particlecounts_DN06}
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/nails}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{nails}
...
sediment. \mrq[ss]{2.2}
%DIF <  The positive gradients in the intertidal beach area therefore suggest
%DIF <  that the intertidal beach is an important source of aeolian
%DIF <  sediment. The small positive gradients over the dry beach similarly
%DIF <  suggest that this area is a minor source of sediment.
...
%DIF <  The negative gradients therefore indicate that deposition of sediment
%DIF <  occurs locally at the transition between intertidal and dry beach.
...
level change confirm that \DIFdelbegin \DIFdel{erosion by wind }\DIFdelend \DIFaddbegin \DIFadd{wind erosion }\DIFaddend is concentrated on the berm
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/particlecounts_distheight}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{particlecounts_distheight}
  \DIFaddendFL \caption{Cumulative particle count distribution over the vertical
    during deployment DN08. The line indicates the percentage of
    particles that \DIFdelbeginFL \DIFdelFL{bypasses }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{surpasses }\DIFaddendFL a certain height above the bed. The
...
\DIFdelbegin \DIFdel{bypassing }\DIFdelend \DIFaddbegin \DIFadd{surpassing }\DIFaddend the measurement equipment. To ensure that the number of
\DIFdelbegin \DIFdel{bypassing }\DIFdelend \DIFaddbegin \DIFadd{surpassing }\DIFaddend particles is limited, the most landward mast in each
...
number of particles \DIFdelbegin \DIFdel{bypassed }\DIFdelend \DIFaddbegin \DIFadd{surpassed }\DIFaddend the equipment at this point.
...
direction. %DIF < , which seems in accordance with the development of a
%DIF < boundary layer
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/particlecounts_velocities}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{particlecounts_velocities}
...
\citep{Hoonhout2017a}. Armoring of the dry beach surface, due to \DIFdelbegin \DIFdel{formation of lag deposits}\DIFdelend \DIFaddbegin \DIFadd{the
emergence of shells and other roughness elements}\DIFaddend , might lead to a
...
\begin{figure}
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/armoring_small}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{armoring_small}
  \DIFaddendFL \caption{Visual impression of armor layer at three locations in the
...
\DIFdelbegin \DIFdel{pavement }\DIFdelend \DIFaddbegin \DIFadd{pavements }\DIFaddend emerged from the nourished sediment in the first half year
after construction of the Sand Motor \citep{Hoonhout2017a} due to
winnowing of sand from the bed. \DIFaddbegin \DIFadd{The exact influence of a shell
pavement on saltation is challenging to assess. }\DIFaddend Roughness elements,
...
\begin{figure}
 \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{../Figures/temporal_deposits}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{temporal_deposits}
  \DIFaddendFL \caption{Conceptual illustration of how temporal deposits facilitate
...
lagoon is a phased process. \DIFdelbegin \DIFdel{In a }\DIFdelend \DIFaddbegin \DIFadd{Otherwise the sediment supply to the dunes
would cease during high water as the main sediment source is
flooded. In an }\DIFaddend phased system the local sediment deposits at the berm
...
capacity emphasized by \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{Houser2009, Anthony2013}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{Houser2009}}%DIFAUXCMD
}\DIFaddend .

% water level
%DIF < Low average water levels accommodate initiation of aeolian sediment
%DIF < transport relatively distant from the high water line where the
%DIF < overall particle count tends to peak. As average water levels increase
%DIF < with increasing wind speed, due to wind setup, the available fetch
%DIF < decreases. Nevertheless, the overall particle count still peaks close
%DIF < to the high water line and not beyond. Moreover, the height of the
%DIF < peak in overall particle count increases with increasing wind
%DIF < speed. Both the decrease in available fetch and the increase in
%DIF < maximum overall particle count result in an increase in overall
%DIF < particle count gradient over the intertidal beach.  The increased
%DIF < gradient indicates that also the rate of sediment entrainment in the
%DIF < intertidal beach area increases and overcompensates the decrease in
%DIF < fetch. Also the entrainment rate at the dry beach increases with
%DIF < increasing wind speed, but to a significantly less extent than at the
%DIF < intertidal beach. The exception is again the storm deployment DN10,
%DIF < which shows a peak in overall particle count landward of the high
%DIF < water line.
\DIFaddbegin \DIFadd{Low average water levels accommodate initiation of aeolian sediment
...
%DIF <  high wind events
%DIF < The per-mast particle count obtained during the storm deployment DN10
%DIF < appeared to be exceptional with respect to the other non-storm
%DIF < deployments: the overall particle count during deployment DN10 was low
%DIF < compared to the prevailing wind speed and direction, the negative
%DIF < gradients in per-mast particle count at the transition between
%DIF < intertidal and dry beach were absent and the peak in overall particle
%DIF < count did not coincide with the high water line or berm edge.
\DIFdelbegin %DIFDELCMD < 
...
availability \mbox{%DIFAUXCMD
\citep{McKennaNeuman2012} }%DIFAUXCMD
and cause spatial variations in
aeolian sediment supply \mbox{%DIFAUXCMD
\citep{Jackson2010}}%DIFAUXCMD
...
\DIFdel{the high number of shells and other roughness elements that is
  generally contained by nourishment sand \mbox{%DIFAUXCMD
\citep{VanDerWal1998,
    VanDerWal2000}}%DIFAUXCMD
, and
}%DIFDELCMD < \item %%%
